		...
		struct mq_attr *attr)
{
	...
	if (S_ISREG(mode)) {
		...
		if (capable(CAP_SYS_RESOURCE)) {
			if (info->attr.mq_maxmsg > DROID_LKM_HARD_MSGMAX ||
			    info->attr.mq_msgsize > DROID_LKM_HARD_MSGSIZEMAX)
				goto out_inode;
		} else {
		      ...
		      struct mq_attr *attr)
{
	...
	if (IS_ERR(path.dentry)) {
		...
		droid_lkm_dbg("mq open: lookup name=%s len=%zu -> %d\n",
			      name->name, strlen(name->name), error);
		goto out_putfd;
	}
	path.mnt = mntget(mnt);
	error = prepare_open(path.dentry, oflag, ro, mode, name, attr);
	if (error)
		droid_lkm_dbg("mq open: prepare %s ro=%d -> %d\n",
			      name->name, ro, error);
	if (!error) {
...
int droid_lkm_mq_init_ns(struct ipc_namespace *ns)
{
	...
	ns->mq_queues_max    = DROID_LKM_DFLT_QUEUESMAX;
	ns->mq_msg_max       = DROID_LKM_DFLT_MSGMAX;
	ns->mq_msgsize_max   = DROID_LKM_DFLT_MSGSIZEMAX;
	ns->mq_msg_default   = DROID_LKM_DFLT_MSG;
	ns->mq_msgsize_default  = DROID_LKM_DFLT_MSGSIZE;
...
int __init droid_lkm_mqueue_fs_init(void)
{
	...
	error = droid_lkm_mqueue_shim_init();
	if (error)
		return error;
