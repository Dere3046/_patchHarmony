...
static struct proc_dir_entry *droid_lkm_sysvipc_dir;

void droid_lkm_ipc_proc_exit(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(droid_lkm_ipc_proc_slots); i++) {
		if (!droid_lkm_ipc_proc_slots[i].pde)
			continue;
		proc_remove(droid_lkm_ipc_proc_slots[i].pde);
		droid_lkm_ipc_proc_slots[i].pde = NULL;
	}

	if (droid_lkm_sysvipc_dir) {
		proc_remove(droid_lkm_sysvipc_dir);
		droid_lkm_sysvipc_dir = NULL;
	}
}
#else
void droid_lkm_ipc_proc_exit(void) {}
...
static int ipc_init(void)
{
	...
	droid_lkm_sysvipc_dir = proc_mkdir("sysvipc", NULL);
#endif
	sem_init();
	msg_init();
	shm_init();
		...
		int ids, int (*show)(struct seq_file *, void *))
{
	...
	for (i = 0; i < ARRAY_SIZE(droid_lkm_ipc_proc_slots); i++) {
		if (!strcmp(droid_lkm_ipc_proc_slots[i].path, path))
			break;
	}
	if (i < ARRAY_SIZE(droid_lkm_ipc_proc_slots) && droid_lkm_ipc_proc_slots[i].pde)
		return;
	...
	if (i < ARRAY_SIZE(droid_lkm_ipc_proc_slots))
		droid_lkm_ipc_proc_slots[i].pde = pde;
}
#endif
