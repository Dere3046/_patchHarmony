...
void droid_lkm_exit_shm_impl(struct task_struct *task)
{
	for (;;) {
		struct shmid_kernel *shp;
		struct ipc_namespace *ns;
		...
		if (list_empty(droid_lkm_task_shm_clist(task))) {
			task_unlock(task);
			break;
		}

		shp = list_first_entry(droid_lkm_task_shm_clist(task), struct shmid_kernel,
				shm_clist);
...
static int newseg(struct ipc_namespace *ns, struct ipc_params *params)
{
	...
	if (shmflg & SHM_HUGETLB) {
		 ...
		 */
		if  ((shmflg & SHM_NORESERVE) &&
				droid_lkm_overcommit_memory() != OVERCOMMIT_NEVER)
			acctflag = VM_NORESERVE;
	...
	task_lock(current);
	list_add(&shp->shm_clist, droid_lkm_task_shm_clist(current));
	task_unlock(current);
 ...
 * this.
 */
long droid_lkm_do_shmat(int shmid, char __user *shmaddr, int shmflg,
	      ulong *raddr, unsigned long shmlba)
{
	...
	addr = droid_lkm_do_mmap(file, addr, size, prot, flags, 0, 0, &populate, NULL);
	*raddr = addr;
	err = 0;
...
SYSCALL_DEFINE3(shmat, int, shmid, char __user *, shmaddr, int, shmflg)
{
	...
	err = droid_lkm_do_shmat(shmid, shmaddr, shmflg, &ret, SHMLBA);
	if (err)
		return err;
...
COMPAT_SYSCALL_DEFINE3(shmat, int, shmid, compat_uptr_t, shmaddr, int, shmflg)
{
	...
	err = droid_lkm_do_shmat(shmid, compat_ptr(shmaddr), shmflg, &ret, COMPAT_SHMLBA);
	if (err)
		return err;
	force_successful_syscall_return();
	return (long)ret;
...
/*
 * stable entry for the deferred cleanup in ipc_shim.c
 */
void droid_lkm_exit_shm(struct task_struct *task)
{
	droid_lkm_exit_shm_impl(task);
}
