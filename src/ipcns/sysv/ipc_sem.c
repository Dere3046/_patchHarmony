...
static inline int get_undo_list(struct sem_undo_list **undo_listp)
{
	...
	undo_list = droid_lkm_task_undo_list(current);
	if (!undo_list) {
		...
		droid_lkm_task_undo_list_set(current, undo_list);
	}
...
int droid_lkm_copy_semundo(unsigned long clone_flags, struct task_struct *tsk)
{
	struct sem_undo_list *undo_list;
	int error;
	...
	if (clone_flags & CLONE_SYSVSEM) {
		...
		droid_lkm_task_undo_list_set(tsk, undo_list);
	} else
		droid_lkm_task_undo_list_set(tsk, NULL);
 ...
 */
void droid_lkm_exit_sem_impl(struct task_struct *tsk, struct ipc_namespace *ns)
{
	struct sem_undo_list *ulp;

	
	if (!ns)
		ns = droid_lkm_ipcns_task_ns(tsk);

	ulp = droid_lkm_task_undo_list(tsk);
	if (!ulp)
		return;
	droid_lkm_task_undo_list_set(tsk, NULL);
